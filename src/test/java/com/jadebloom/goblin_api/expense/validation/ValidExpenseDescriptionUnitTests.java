...
public class ValidExpenseDescriptionUnitTests {
    ...
    public void givenValidDescription_whenValidating_thenDoNotThrow() {
        ...
        assertAll("Assert that valid expense descriptions can be validated",
    ...
    @Test
    public void givenValidDescription_whenValidating_thenThrowInvalidExpenseDescription() {
        Foo f1 = new Foo("");
        Foo f2 = new Foo("    ");
        Foo f3 = new Foo(
        ...
        assertAll("Assert that invalid expense descriptions can be validated",
                () -> assertThrowsExactly(InvalidExpenseDescription.class, () -> validate(f1)),
                () -> assertThrowsExactly(InvalidExpenseDescription.class, () -> validate(f2)),
                () -> assertThrowsExactly(InvalidExpenseDescription.class, () -> validate(f3)));
    }
    ...
    private class InvalidExpenseDescription extends RuntimeException {
    }

    private void validate(Foo foo) {
        if (!GenericValidator.isValid(foo)) {
            throw new InvalidExpenseDescription();
        }
    }
