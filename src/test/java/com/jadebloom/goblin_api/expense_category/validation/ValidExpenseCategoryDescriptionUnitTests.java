package com.jadebloom.goblin_api.expense_category.validation;
...
public class ValidExpenseCategoryDescriptionUnitTests {
	...
	public void GivenValidDescription_WhenValidating_thenDoNotThrow() {
		...
		assertAll("Assert that valid expense category descriptions can be validated",
				...
				() -> assertDoesNotThrow(() -> validate(f4)));
	}

	@Test
	public void GivenInvalidDescription_WhenValidating_ThenThrowInvalidExpenseCategoryDescription() {
		Foo f1 = new Foo("    ");
		Foo f2 = new Foo(
		...
		assertAll("Assert that invalid expense category descriptions can be validated",
				() -> assertThrowsExactly(InvalidExpenseCategoryDescription.class,
						() -> validate(f1)),
				() -> assertThrowsExactly(InvalidExpenseCategoryDescription.class,
						() -> validate(f2)));
	}
